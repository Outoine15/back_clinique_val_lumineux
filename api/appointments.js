...
async function handlePut(splittedRoute, headers, data, query) {
    ...
    } else if(splittedRoute.length == 2) {
        if(splittedRoute[1] == "subscribe") {
            content = await subscribeAppointments(headers, data, splittedRoute[0], query);
        } else if(splittedRoute[1] == "update") {
            content = await updateAppointments(headers, data, splittedRoute[0], query);
        }
    }
...
async function createAppointments(headers, data, query) { // création d'un rendez-vous de la part d'un docteur ou d'une secretaire
    var res = {}; 
